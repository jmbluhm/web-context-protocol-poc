...
/**
 * POST handler for subscription changes
 * This endpoint is called by the LLM when user wants to modify subscription
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (forwardingUrl) {
      try {
        ...
        if (forwardResponse.ok) {
          ...
          return new NextResponse(
            'Subscription change request forwarded and processed successfully.',
            {
              status: 200,
              headers: {
                'Content-Type': 'text/plain',
          ...
          return new NextResponse(
            'Subscription change request received but forwarding failed.',
            {
              status: 200,
              headers: {
                'Content-Type': 'text/plain',
      ...
      return new NextResponse(
        'Subscription change request received and processed successfully.',
        {
          status: 200,
          headers: {
            'Content-Type': 'text/plain',
    ...
    console.error('Error in change API:', error);
    return new NextResponse(
      'Internal server error occurred while processing change request.',
      {
        status: 500,
        headers: {
          'Content-Type': 'text/plain',
